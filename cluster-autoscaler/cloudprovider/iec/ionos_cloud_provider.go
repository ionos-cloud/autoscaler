...
func BuildIECCloudProvider(manager IECManager, rl *cloudprovider.ResourceLimiter) *IECCloudProvider {
...
func (d *IECCloudProvider) NodeGroups() []cloudprovider.NodeGroup {
	ng := d.manager.GetNodeGroups()
	nodeGroups := make([]cloudprovider.NodeGroup, len(ng))
	for i, ng := range ng {
		nodeGroups[i] = ng
	}
	return nodeGroups
...
func BuildIEC(
	...
	configFile, err := read(opts.CloudConfig)
	if err != nil {
		logFatal("Couldn't read cloud provider configuration %s: %v", opts.CloudConfig, err)
	}
	manager, err := createIECManager(bytes.NewReader(configFile))
	if err != nil {
