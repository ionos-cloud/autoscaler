...
func TestLoadConfigFromEnv(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			env:       map[string]string{envKeyClusterID: "1", envKeyToken: "token", envKeyCustomHeaders: "foo=bar,baz=qux"},
			expectErr: true,
		},
		{
			name: "use defaults",
		...
		},
		{
			...
			env: map[string]string{
				...
				envKeyCustomHeaders: "foo:bar;baz:qux",
			},
			expectCfg: &Config{
				ClusterID:     "test",
				Endpoint:      "/dev/null",
				...
				CustomHeaders: map[string]string{"foo": "bar", "baz": "qux"},
			},
		},
	}
